...
Packstack recently launched an [Ultralight Gear Research tool](/tools/ultralight-research) that lets you browse thousands of products across every backpacking gear category, sorted by weight from lightest to heaviest. You can filter by brand or weight range, toggle between metric and imperial, and expand any product to see variant weights and detailed specs — materials, R-values, packed dimensions, temperature ratings, and more. Each row shows how the product compares to the category median weight, so you can immediately see whether something is meaningfully light or just average.
...
- **OutPack** — A web app that combines itemized packing lists with features to catalogue trips, add notes, mark waypoints, and upload photos. Think LighterPack meets a trail journal. It was featured in Backpacker Magazine as the best new trip planner.
...
- **Gearli.io** — Helps hikers and backpackers organise their gear, create reusable gear bags, plan trips, track pack weight, and import gear lists from LighterPack, PackLight, Google Sheets, and Excel.
